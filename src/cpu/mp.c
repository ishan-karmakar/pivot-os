...
void start_aps(madt_t *madt) {
    ...
    log(Verbose, true, "MP", "This processor's APIC ID is %u", current_apic_id);
    ...
    while (lapic != NULL) {
        ...
        if (apic_id != current_apic_id) {
            log(Info, true, "MP", "Starting up processor %u", apic_id);
...
void start_ap(uint32_t id, uint8_t trampoline_page __attribute__((unused))) {
    ...
    *processor_status = 0;
    // printf("a\n");
    log(Verbose, true, "MP", "Sending INIT to processor");
