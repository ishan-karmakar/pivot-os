...
fn vmm_init() kernel.Task.Ret {
    ...
    const vmm_size = std.math.ceilPowerOfTwoAssert(usize, pmm.get_free_size() * 3 / 100);
...
fn kheap_init() kernel.Task.Ret {
    ...
    // For now it will be 0.5%
    // It must be less than or equal to the VMM size
    const kheap_size = (pmm.get_free_size() * 2 / 100) / 0x1000 * 0x1000;
