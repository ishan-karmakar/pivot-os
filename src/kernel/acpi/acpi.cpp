...
ACPI::ACPI(uintptr_t rsdp) : SDT{get_rsdt(reinterpret_cast<char*>(rsdp))} {
    log(Info, "ACPI", "Found %cSDT table", xsdt ? 'X' : 'R');
}

template <class T>
std::optional<T> ACPI::get_table() {
...
SDT::sdt *ACPI::get_rsdt(char *sdp) {
    auto rsdp = reinterpret_cast<ACPI::rsdp*>(sdp);
    xsdt = false;
...
template std::optional<MADT> ACPI::get_table();
