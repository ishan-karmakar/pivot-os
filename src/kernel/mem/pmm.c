...
void map_higher_half(page_table_t p4_tbl) {
    ...
    for (size_t i = 0; i < mmap_num_entries; i++) {
        ...
        if ((type == 2 || type == 3 || type == 4 || type == 7) && current_desc->physical_start != 0)
            map_range(current_desc->physical_start, VADDR(current_desc->physical_start), current_desc->count, KERNEL_PT_ENTRY, p4_tbl);
        current_desc = (mmap_descriptor_t*) ((uint8_t*) current_desc + mem_info->mmap_descriptor_size);
    }

    map_range((uintptr_t) mem_info->kernel_entries, VADDR(mem_info->kernel_entries), SIZE_TO_PAGES(mem_info->num_kernel_entries * sizeof(kernel_entry_t)), KERNEL_PT_ENTRY, p4_tbl);
