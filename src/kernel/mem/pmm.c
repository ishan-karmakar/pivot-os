...
void init_pmm(mem_info_t *memory_info) {
    ...
    pmm_set_bit(0x8000); // Needed for SMP
    ...
    for (size_t i = 0; i < mmap_num_entries; i++) {
        ...
        if ((type == 2 || type == 3 || type == 4 || type == 7) && current_desc->physical_start != 0) {
            log(Verbose, "PMM", "%x, %x, %u", current_desc->physical_start, VADDR(current_desc->physical_start), current_desc->count);
            map_range(current_desc->physical_start, VADDR(current_desc->physical_start), current_desc->count, KERNEL_PT_ENTRY, NULL);
        }
        current_desc = (mmap_descriptor_t*) ((uint8_t*) current_desc + mem_info->mmap_descriptor_size);
    }

    map_range((uintptr_t) mem_info->kernel_entries, VADDR(mem_info->kernel_entries), SIZE_TO_PAGES(mem_info->num_kernel_entries * sizeof(kernel_entry_t)), KERNEL_PT_ENTRY, NULL);
...
void map_kernel_entries(page_table_t p4_tbl) {
    for (size_t i = 0; i < mem_info->num_kernel_entries; i++) {
        log(Verbose, "PMM", "%x, %x, %u", mem_info->kernel_entries[i].paddr, mem_info->kernel_entries[i].vaddr, mem_info->kernel_entries[i].num_pages);
        map_range(mem_info->kernel_entries[i].paddr, mem_info->kernel_entries[i].vaddr, mem_info->kernel_entries[i].num_pages, USER_PT_ENTRY, p4_tbl);
    }
}
...
void map_range(uintptr_t physical, uintptr_t virtual, size_t num_pages, size_t flags, page_table_t p4_tbl) {
    for (size_t i = 0; i < num_pages; i++) {
        if (true) {
            log(Verbose, "MAPR", "%x, %x, %b", physical + i * PAGE_SIZE, virtual + i * PAGE_SIZE, pmm_check_bit(0xC2000));
            // while(1);
        }
...
void map_addr(uintptr_t physical, uintptr_t virtual, size_t flags, page_table_t p4_tbl) {
    if (pmm_check_bit(0xC2000)) {
        log(Verbose, "MAPB", "%x, %x, %b", physical, virtual, 1);
        while(1);
    }
    if (p4_tbl == NULL)
    ...
    if (pmm_check_bit(0xC2000)) {
        log(Verbose, "MAP4", "%x, %x, %b", physical, virtual, 1);
        while(1);
    }
    ...
    if (pmm_check_bit(0xC2000)) {
        log(Verbose, "MAP3", "%x, %x, %b", physical, virtual, 1);
        while(1);
    }
    ...
    if (pmm_check_bit(0xC2000)) {
        log(Verbose, "MAP2", "%x, %x, %b", physical, virtual, 1);
        while(1);
    }
...
bool pmm_check_bit(uintptr_t address) {
    address /= PAGE_SIZE;
    return !(mem_info->bitmap[address / BITMAP_ROW_BITS] & (1UL << (address % BITMAP_ROW_BITS)));
}
    ...
    for (uint16_t row = 0; row < mem_info->bitmap_entries; row++) {
        if (mem_info->bitmap[row] != BITMAP_ROW_FULL)
            for (uint16_t col = 0; col < BITMAP_ROW_BITS; col++)
                if (!(mem_info->bitmap[row] & (1UL << col)))
