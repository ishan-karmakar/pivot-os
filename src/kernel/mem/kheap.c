...
static kheap_mem_node_t *kernel_heap_current_pos;
...
static kheap_mem_node_t* create_kheap_node( kheap_mem_node_t *current_node, size_t size ) {
...
static uint64_t compute_kheap_end() {
    return (uint64_t)kernel_heap_end + kernel_heap_end->size + sizeof(kheap_mem_node_t);
}

static uint8_t can_merge(kheap_mem_node_t *cur_node) {
...
static void merge_memory_nodes(kheap_mem_node_t *left_node, kheap_mem_node_t *right_node) {
