...
namespace lib {
    // Copied straight from frg::vector but has erase methods
    template<typename T, typename Allocator = heap::allocator_t>
    class vector {
    public:
        using value_type = T;
    ...
    template<typename T, typename Allocator>
    template<typename... Args>
    void vector<T, Allocator>::resize(std::size_t new_size, Args &&... args) {
    ...
    template <typename T, typename Allocator>
    void vector<T, Allocator>::erase(std::size_t first, std::size_t last) {
        std::size_t n = last - first + 1;
        memmove(_elements + first, _elements + last + 1, sizeof(T) * n);
        _size -= n;
    }

    template<typename T, typename Allocator>
    void vector<T, Allocator>::_ensure_capacity(std::size_t capacity) {
