...
impl<'a> PTMapper<'a> {
    ...
    pub fn new(pml4: usize, pmm: &'a Mutex<PhysicalMemoryManager>) -> Self {
        Self {
            pml4: unsafe { &mut *(virt_addr(pml4) as *mut PageTable) },
            pmm
        }
    }
    ...
    pub fn map(&mut self, phys: usize, virt: usize, mut flags: PageTableFlags) {
        ...
        let p4_ent = unsafe { &mut (*self.pml4)[virta.p4_index()] };
        ...
        let p1_tbl = if p2_ent.flags().contains(PageTableFlags::HUGE_PAGE) {
            if physa.align_down(Self::HP_SIZE as u64) == p2_ent.addr() && hp_works {
                ...
                let tbl_phys = self.pmm.lock().frame();
    ...
    pub fn unmap(&mut self, virt: usize) {
        ...
        let p4_ent = unsafe { &mut (*self.pml4)[virt.p4_index()] };
...
pub fn init(pmm: &Mutex<PhysicalMemoryManager>) -> Mutex<PTMapper> {
    let cr3 = Cr3::read().0.start_address().as_u64() as usize;
    log::debug!("CR3: {:#x}", cr3);
    // KMAPPER.init(cr3);
    log::info!("Initialized kernel mapper");
    Mutex::new(PTMapper::new(cr3, pmm))
}